...
import { PartnerAddFormType, PartnersParamsType } from '@/app/apis/partners/PartnersApi.type';
import { usePatchUpdatePartnerMutation, usePutCreatePartnerMutation } from '@/app/apis/partners/PartnersApi.mutation';
...
function PartnerBasicInfo({
  ...
  useEffect(() => {
    if(info){
      ...
      setValue('mailOrderSalesRegistrationNo', info.mailOrderSalesRegistrationNo);
  ...
  const onSubmit = () => {
    ...
    if(watch('password') != '' && watch('password') != undefined){
      ...
      if(watch('password_check') == '' || watch('password_check') == undefined){
        toast({
          position: 'top',
          duration: 2000,
          render: () => (
    ...
    <>
      ...
      <Flex mt={'30px'} flexDirection={'column'}>
        ...
        <Flex flexDirection={'column'} pb={'20px'}>
          ...
          <Text color={ColorBlack} fontSize={'15px'}>{watch('loginId')}</Text>
          </Flex>
          <Text color={ColorRed} fontSize={'12px'} ml={'167px'}>{error.loginId}</Text>
        </Flex>
        <Grid templateColumns='repeat(2, 1fr)'>
          <GridItem>
          <Flex flexDirection={'row'} pb={'20px'} alignItems={'center'} width={'100%'}>
            <Text
              w={'165px'}
              flexShrink={0}
              color={ColorBlack}
          ...
          <GridItem>
          <Flex flexDirection={'row'} pb={'20px'} alignItems={'center'} ml={'40px'}>
            <Text
              w={'100px'}
              flexShrink={0}
              color={ColorBlack}
