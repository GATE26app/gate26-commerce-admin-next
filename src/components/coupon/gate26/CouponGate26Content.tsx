...
import CouponGate26Table from './CouponGate26Table';
...
function CouponGate26Content() {
  ...
  function handleChangeInput(key: string, value: string | number, id?: string) {
    ...
    if (key === 'limit') {
      ...
      router.push(`/coupon/gate26?page=${Number(value) + 1}`);
      setGate26FilterInfo({
        ...gate26FilterInfo,
        pageNo: Number(value),
      });
  ...
  const { mutate: refreshList, isLoading } = useGetCouponListMutation({
    options: {
      onSuccess: (res) => {
        console.log('ffffffffffff0', res.data);
        setList(res.data);
        setGoodsInfo({
  ...
  useEffect(() => {
    if (goodsInfo.gate26CouponState) refreshList(request);
  }, [goodsInfo.gate26CouponState]);
    ...
    <Box mt={'40px'}>
      <Flex flexDirection={'row'} justifyContent={'space-between'}>
        ...
        <Flex
          ...
          borderRadius={'10px'}
          py={'12px'}
          onClick={() => router.push('/coupon/gate26/create')}
        >
...
export default CouponGate26Content;
