...
import CouponPartnerTable from './CouponPartnerTable';
function CouponPartnerContent() {
  ...
  const [request, setRequest] = useState<CouponListResType>({
    ...
    type: 2, //1=>전체상품(gate26전체상품), 2=>특정상점(파트너사쿠폰), 3=>특정상품(단일상품)
    access: 0,
  });
  ...
  useEffect(() => {
    if (goodsInfo.partnerCouponState) refreshList(request);
  }, [goodsInfo.partnerCouponState]);
    ...
    <Box mt={'40px'}>
      <Flex flexDirection={'row'} justifyContent={'space-between'}>
        ...
        <Flex
          ...
          borderRadius={'10px'}
          py={'12px'}
          onClick={() => router.push('/coupon/partner/create')}
        >
...
export default CouponPartnerContent;
