...
function CouponPermissionUser({
  ...
  console.log('ClickUserList', ClickUserList);
  console.log('clickData', clickData);
  useEffect(() => {
    console.log('CouponData.permissions', CouponData.permissions);
    if (
      CouponData.permissions !== undefined &&
      CouponData.permissions?.length > 0
    ) {
      let array: string[] = [];
      CouponData.permissions.map((item) => {
        console.log('vvvv', item.user.userId);
        array.push(item.user.userId);
      });
      setClickData([CouponData.permissions[0].user]);

      setClickUserList(array);
    }
  }, [CouponData.permissions]);
  const {
    data: userListData,
    error,
    isFetching,
